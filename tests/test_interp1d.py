...
def test_interp1d_big(dtype):
    x = np.array([0.0, 0.8, 0.3, 0.05, 0.6, 0.9,
                 0.5, 0.2, 0.0, 0.7, 1.0, 0.4, 1.1], dtype=dtype)
    y = x**2
    expected_linear = np.array([
        0.015, 0.0275, 0.04, 0.065, 0.09, 0.125, 0.16, 0.205, 0.25, 0.305,
        0.36, 0.425, 0.49, 0.565, 0.64, 0.725, 0.81, 0.905, 1., 1.105, 1.21
    ], dtype=dtype)
    expected_spline = np.array([
        0.01, 0.0225, 0.04, 0.0625, 0.09, 0.1225, 0.16, 0.2025, 0.25, 0.3025,
        0.36, 0.4225, 0.49, 0.5625, 0.64, 0.7225, 0.81, 0.9025, 1.0, 1.1025, 1.21
    ], dtype=dtype)
...
def test_interp1d_empty():
    ...
    with pytest.raises(ValueError):
        interp1d(np.array([], dtype=np.float32), np.array([], dtype=np.float32), x_new, 'linear')
    with pytest.raises(ValueError):
        interp1d(np.array([0.1], dtype=np.float32), np.array([], dtype=np.float32), x_new, 'linear')
    with pytest.raises(ValueError):
        interp1d(np.array([], dtype=np.float32), np.array([0.01], dtype=np.float32), x_new, 'linear')
