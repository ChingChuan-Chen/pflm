...
def test_interp2d_happy_case(dtype):
    ...
    x = np.array([0., 1., 3., 4., 5.], dtype=dtype)
    y = np.array([10., 11., 12., 13.], dtype=dtype)

    x_new = np.linspace(0., 5., 7, dtype=dtype)
    y_new = np.linspace(10., 13., 7, dtype=dtype)
...
def test_interp2d_unsorted(dtype):
    A = np.array([[13., -1., 12.], [5., 4., 3.], [1., 6., 2.]], dtype=dtype)
    x = np.array([0., 1., 4.], dtype=dtype)
    y = np.array([10., 11., 12.], dtype=dtype)
    x_new = np.linspace(0., 4., 4, dtype=dtype)
    y_new = np.linspace(10., 12., 4, dtype=dtype)
...
def test_interp2d_two_points(dtype):
    A = np.array([[13., -1.], [5., 4.]], dtype=dtype)
    x = np.array([0., 1.], dtype=dtype)
    y = np.array([10., 11.], dtype=dtype)
    expected = np.array([
...
def test_interp2d_spline_small_size(dtype):
    ...
    A1 = np.array([[13., -1., 12., -5.], [8., 2., 7., -4.], [5., 4., 3., 1.], [1., 6., 2., 4.]], dtype=dtype)
    x1 = np.array([0., 1., 3., 4.], dtype=dtype)
    y1 = np.array([10., 11., 12., 13.], dtype=dtype)
    x_new1 = np.linspace(0., 4., 6, dtype=dtype)
    y_new1 = np.linspace(10., 13., 6, dtype=dtype)
    assert_allclose(interp2d(x1, y1, A1, x_new1, y_new1, 'spline'), expected1, rtol=1e-5, atol=1e-6)

    A2 = np.array([[13., -1., 12.], [5., 4., 3.], [1., 6., 2.]], dtype=dtype)
    x2 = np.array([0., 1., 4.], dtype=dtype)
    y2 = np.array([10., 11., 12.], dtype=dtype)
    x_new2 = np.linspace(0., 4., 4, dtype=dtype)
    y_new2 = np.linspace(10., 12., 4, dtype=dtype)
    assert_allclose(interp2d(x2, y2, A2, x_new2, y_new2, 'spline'), expected2, rtol=1e-5, atol=1e-6)

    A3 = np.array([[13., -1.], [5., 4.]], dtype=dtype)
    x3 = np.array([0., 1.], dtype=dtype)
    y3 = np.array([10., 11.], dtype=dtype)
    x_new3 = np.linspace(0., 1., 4, dtype=dtype)
    y_new3 = np.linspace(10., 11., 4, dtype=dtype)
...
def test_interp2d_type_mismatch():
    x_f64 = np.array([0., 1., 4.], dtype=np.float64)
    y_f32 = np.array([10., 11., 12.], dtype=np.float32)
    v_f32 = np.array([[13., -1., 12.], [5., 4., 3.], [1., 6., 2.]], dtype=np.float32)
    x_new_f32 = np.linspace(0., 4., 4, dtype=np.float32)
    y_new_f64 = np.linspace(10., 12., 4, dtype=np.float64)
