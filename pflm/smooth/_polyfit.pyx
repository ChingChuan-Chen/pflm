...
def calculate_kernel_value_f64(
    ...
    """calculate_kernel_value_f64(u, kernel_type, wj) -> np.float64_t (test only)"""
    if kernel_type >= 4 and abs(u) > 1.0:
        return 0.0
    if kernel_type == 10 and abs(abs(u) - 1.0) <= 1e-7:  # Cosine kernel is zero at the boundaries
        return 0.0
    return _calculate_kernel_value_f64(u, kernel_type) * wj
...
def calculate_kernel_value_f32(
    ...
    """calculate_kernel_value_f32(u, kernel_type, wj) -> np.float32_t (test only)"""
    if kernel_type >= 4 and abs(u) > 1.0:
        return 0.0
    if kernel_type == 10 and abs(abs(u) - 1.0) <= 1e-7:  # Cosine kernel is zero at the boundaries
        return 0.0
    return _calculate_kernel_value_f32(u, kernel_type) * wj
...
cdef void polyfit1d_helper_f64(
    ...
    cdef np.float64_t center_minus_xj = 0.0, u = 0.0, sqrt_wj = 0.0
    ...
    for j in range(left, right):
        center_minus_xj = x[j] - center
        u = center_minus_xj / bw
        sqrt_wj = sqrt(_calculate_kernel_value_f32(u, kernel_type) * w[j])

        ly[i] = y[j] * sqrt_wj
        lx[i] = sqrt_wj
        for k in range(1, degree + 1):
            lx[i + n_rows * k] = pow(center_minus_xj, k) * sqrt_wj
        i += 1
    cdef int info = 0
    _gels_helper(
...
cdef void polyfit1d_helper_f32(
    ...
    cdef np.float32_t center_minus_xj = 0.0, u = 0.0, sqrt_wj = 0.0
