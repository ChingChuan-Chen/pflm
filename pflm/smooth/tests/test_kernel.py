...
def test_calculate_kernel_value(dtype, func):
    ...
    result = np.array([func(ui, KernelType.GAUSSIAN_VAR.value) for ui in u])
    expected = (1 / np.sqrt(2 * np.pi)) * np.exp(-0.5 * u**2) * (1.25 - 0.25 * u**2)
    assert_allclose(result, expected, rtol=1e-5, atol=0.0)
...
def test_calculate_kernel_value_gausvar(dtype, func):
    u = np.linspace(-4.0, 4.0, 161, dtype=dtype)

    # Test Gaussian variance kernel with large u
    result = np.array([func(ui, KernelType.GAUSSIAN_VAR.value) for i, ui in enumerate(u)])
    expected = np.zeros_like(u)
    u_sq = u**2
    idx = np.nonzero(u_sq < 5.0)
    expected[idx] = (1 / np.sqrt(2 * np.pi)) * np.exp(-0.5 * u_sq[idx]) * (1.25 - 0.25 * u_sq[idx])
    assert_allclose(result, expected, rtol=1e-5, atol=0.0)
