...
void interp1d_linear(T x[], T y[], T x_new[], T y_new[], std::ptrdiff_t x_size, std::ptrdiff_t x_new_size) {
  ...
  for (i = 0; i < x_new_size; ++i) {
    j = search_sorted<T>(x, x_size, x_new[i]) - 1;
    if (j >= 0) {
...
void interp2d_linear(
  ...
  #pragma omp parallel for private(i)
  for (i = 0; i < x_new_size; ++i) {
    ptrdiff_t x_idx = search_sorted(x, x_size, x_new[i]) - 1;
    for (ptrdiff_t j = 0; j < y_new_size; ++j) {
      ptrdiff_t y_idx = search_sorted(y, y_size, y_new[j]) - 1;
