...
from pflm.utils.lapack_helper import _posv_memview_f32, _posv_memview_f64


@pytest.mark.parametrize("dtype, posv_func", [(np.float64, _posv_memview_f64), (np.float32, _posv_memview_f32)])
@pytest.mark.parametrize("order", ["C", "F"])
def test_posv_matrix_lower_triangular_matrix(dtype, posv_func, order):
    A = np.array(
        [
    ...
    info = posv_func(BLAS_Uplo.Lower, A, b, 5, 5, 1)
    assert info == 0
    assert_allclose(
        b.ravel(order=order),
...
@pytest.mark.parametrize("dtype, posv_func", [(np.float64, _posv_memview_f64), (np.float32, _posv_memview_f32)])
@pytest.mark.parametrize("order", ["C", "F"])
def test_posv_matrix_upper_triangular_matrix(dtype, posv_func, order):
    A = np.array(
        [
    ...
    b = np.array([[-7.29], [9.25], [5.99], [-1.94], [-8.3]], dtype=dtype, order=order)
    info = posv_func(BLAS_Uplo.Upper, A, b, 5, 5, 1)
    assert info == 0
    assert_allclose(
        b.ravel(order=order),
        np.array([-6.023842441873, 15.618630181394, 3.022190200411, 3.251997882301, -8.783156682244], dtype=dtype),
...
@pytest.mark.parametrize("dtype, posv_func", [(np.float64, _posv_memview_f64), (np.float32, _posv_memview_f32)])
@pytest.mark.parametrize("order", ["C", "F"])
def test_posv_matrix_nrhs_greater_than_1(dtype, posv_func, order):
    A = np.array(
        [
            [3.14, 0.0, 0.0, 0.0, 0.0],
            [0.17, 0.79, 0.0, 0.0, 0.0],
    ...
    info = posv_func(BLAS_Uplo.Lower, A, b, 5, 5, 3)
    assert info == 0
    assert_allclose(
        b,
        np.array(
