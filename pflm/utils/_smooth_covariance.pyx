...
cdef void rotate_polyfit2d_helper(
    ...
    for i in range(n_rows):
        ...
        ly[i] = y[j] * sqrt_wj
        lx[i] = 1.0
        lx[i + n_rows * 1] = pow(x1j_minus_center1 * sqrt_wj, 2)
...
cdef void rotate_polyfit2d_memview(
    ...
    cdef int64_t i, x_new_size = new_grid.shape[0]
    for i in prange(x_new_size, nogil=True):
        rotate_polyfit2d_helper(bandwidth, new_grid[i, 0], new_grid[i, 1], &mu[i], x_grid, y, w, degree, kernel_type)
...
def rotate_polyfit2d_f64(
    ...
    cdef int64_t n_new = new_grid.shape[0]
    cdef np.ndarray[np.float64_t] mu = np.empty(n_new, dtype=np.float64)
    cdef np.ndarray[np.float64_t] rotate_matrix = np.array([[inv_sqrt_2, -inv_sqrt_2], [inv_sqrt_2, inv_sqrt_2]], dtype=np.float64)
    cdef np.ndarray[np.float64_t, ndim=2] x_grid_rotated = rotate_matrix @ x_grid
    cdef np.ndarray[np.float64_t, ndim=2] new_grid_rotated = rotate_matrix @ new_grid

    # create views for Cython
    cdef np.float64_t[:, ::1] x_grid_rotated_view = x_grid_rotated
    cdef np.float64_t[:] y_view = y
    cdef np.float64_t[:] w_view = w
    cdef np.float64_t[:, ::1] new_grid_rotated_view = new_grid_rotated
    cdef np.float64_t[:] mu_view = mu
    rotate_polyfit2d_memview(x_grid_rotated_view, y_view, w_view, new_grid_rotated_view, mu_view, bandwidth, kernel_type, degree)
    return mu
...
def rotate_polyfit2d_f32(
    ...
    cdef np.ndarray[np.float32_t] rotate_matrix = np.array([[inv_sqrt_2, -inv_sqrt_2], [inv_sqrt_2, inv_sqrt_2]], dtype=np.float32)
    cdef np.ndarray[np.float32_t, ndim=2] x_grid_rotated = rotate_matrix @ x_grid
    cdef np.ndarray[np.float32_t, ndim=2] new_grid_rotated = rotate_matrix @ new_grid

    # create views for Cython
    cdef np.float32_t[:, ::1] x_grid_rotated_view = x_grid_rotated
    cdef np.float32_t[:] y_view = y
    cdef np.float32_t[:] w_view = w
    cdef np.float32_t[:, ::1] new_grid_rotated_view = new_grid_rotated
